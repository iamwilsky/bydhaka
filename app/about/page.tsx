import type { Metadata } from 'next'
import { getDealerInfo } from '@/lib/data'
import { Layout } from '@/components/layout/Layout'
import { AboutHero } from '@/components/about/AboutHero'
import { SalesProfile } from '@/components/about/SalesProfile'
import { FacilitiesGrid } from '@/components/about/FacilitiesGrid'
import { LocationMap } from '@/components/about/LocationMap'

export async function generateMetadata(): Promise<Metadata> {
    const dealerInfo = await getDealerInfo()

    return {
        title: `Tentang ${dealerInfo.dealerName} - Sales Consultant BYD Terpercaya`,
        description: `Profil Dealer Resmi ${dealerInfo.dealerName} di ${dealerInfo.address}. Kami siap melayani pembelian mobil listrik BYD. Hubungi ${dealerInfo.salesName} sekarang.`,
        keywords: ['BYD Jakarta', 'BYD Kebon Jeruk', 'Dealer BYD Jakarta', 'Sales BYD Jakarta'],
    }
}

export default async function AboutPage() {
    return (
        <Layout>
            <div className="bg-slate-50 dark:bg-slate-900 transition-colors duration-300 min-h-screen">
                <AboutHero />
                <SalesProfile />
                <FacilitiesGrid />
                <LocationMap />
            </div>
        </Layout>
    )
}
